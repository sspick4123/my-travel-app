...
export default function MyBookmarks({ category, uid }) {
  ...
  if (posts.length === 0) {
    return <div className="text-center text-gray-400">북마크한 글이 없습니다.</div>;
  }

  return (
    <div className="grid gap-6">
      {posts.map((post) => (
        <PostCard key={post.id} post={post} postType={category} />
      ))}
    </div>
  );
}
